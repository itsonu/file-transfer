...
def analyze_tcp(packet):
    ...
    if packet.haslayer(TLS):
        tls_data = packet[TLS]
        print(f"TLS Packet from {src_ip}:{src_port} to {dst_ip}:{dst_port}")
        if TLSHandshake in tls_data:
            print("TLS Handshake:")
            for layer in tls_data[TLSHandshake].field_names:
                print(f"{layer}: {tls_data[TLSHandshake].getlayer(layer).summary()}")
        if TLSRecord in tls_data:
            print("TLS Record:")
            for layer in tls_data[TLSRecord].field_names:
                print(f"{layer}: {tls_data[TLSRecord].getlayer(layer).summary()}")
    elif dst_port == 22:
